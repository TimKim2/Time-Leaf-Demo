...
public class ReactionCollectionEditor : EditorWithSubEditors<ReactionEditor, Reaction>
{
    ...
    public override void OnInspectorGUI ()
    {
		...
		IndexSelectionGUI (rightAreaRect);
	...
	private void IndexSelectionGUI (Rect containingRect)
	{
		Rect topHalf = containingRect;
		topHalf.height *= 0.5f;
		...
		reactionIndex = EditorGUI.IntField (topHalf, reactionIndex);
		//Debug.Log (reactionIndex);
		//reactionIndex = EditorGUI.IntPopup

		if (GUI.Button (bottomHalf, "Insert Selected Reaction"))
		{
			Type reactionType = reactionTypes[selectedIndex];
			Reaction newReaction = ReactionEditor.CreateReaction (reactionType);
			reactionsProperty.InsertToObjectArray(newReaction, reactionIndex);
